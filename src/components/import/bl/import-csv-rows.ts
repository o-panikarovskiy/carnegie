...
const importCSVRows = async <T>(
  ...
  for (let idx = 0; idx < total; idx++) {
    ...
    try {
      ...
      const payload: Payload<T> = { importToken, rowNum, progress, item };
      items.push(item);
      sendToSocket<Payload<T>>(creator.email, { event, payload });
    } catch (error) {
      const payload: Payload<T> = { importToken, rowNum, progress, raw, error: parseError(error) };
      sendToSocket<Payload<T>>(creator.email, { event, payload });
    }
  }

  const payload: Payload<T> = { importToken, rowNum: total, progress: 100 };
